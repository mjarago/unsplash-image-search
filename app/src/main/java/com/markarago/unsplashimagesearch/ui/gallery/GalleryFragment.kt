    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter.addLoadStateListener { loadState ->
            binding.apply {
                ...
                buttonRetry.isVisible = loadState.source.refresh is LoadState.Error // no internet connection
                textViewError.isVisible = loadState.source.refresh is LoadState.Error // no internet connection

                // empty view
                if (loadState.source.refresh is LoadState.Loading &&
                        loadState.append.endOfPaginationReached &&// no more results to be loaded
                        adapter.itemCount < 1) { // no data
